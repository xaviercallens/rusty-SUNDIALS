...
fn gradient_angle(g64: f64, g32: f64) -> f64 {
    if g64 == 0.0 && g32 == 0.0 { return 0.0; }
    let dot = g64 * g32;
    let norm_prod = g64.abs() * g32.abs();
    if norm_prod < 1e-15 { return 0.0; }
...
async fn main() {
    ...
    println!("  Sensitivity FD check: FD={:.5}, Aug={:.5}, diff={:.2e}",
             fd_sensitivity, aug_sensitivity, (fd_sensitivity - aug_sensitivity).abs());


    println!("\n  Initial θ₀ = {:.3} rad, ω₀ = {:.1}, κ₀ = {:.2}", theta0, omega0, kappa);
    println!("\n  {:>4}  {:>8}  {:>10}  {:>10}  {:>12}  {:>8}",
    ...
    for step in 1..=5 {
        let k1 = kappa; let k2 = kappa;
        let t0_1 = theta0; let t0_2 = theta0;
        let o0_1 = omega0; let o0_2 = omega0;

        // Concurrently: FP64 sensitivity (physics) + FP32 ghost (simulated GPU)
        let physics_task = tokio::task::spawn_blocking(move || {
            solve_sensitivity_fp64(k1, t0_1, o0_1)
        });
        ...
        if angle >= 45.0 { all_angles_ok = false; }
        if step > 1 && loss > prev_loss + 0.01 { monotone_ok = false; }
        prev_loss = loss;

        println!("  {:>4}  {:>8.3}  {:>10.5}  {:>10.5}  {:>12.5}  {:>7.3}°  {}",
                 step, kappa, theta_t, loss, dtheta_dk, angle,
                 if angle < 45.0 { "✓" } else { "✗" });
    ...
    println!("══════════════════════════════════════════════════════════════");
    println!(" Underdamped baseline (κ=0.05): θ(T={}) = {:.5} rad", T_END, theta_baseline);
    println!(" Optimised    result  (κ={:.2}): θ(T={}) = {:.5} rad", kappa, T_END, theta_final);
    println!(" Improvement Δ|θ|     = {:.5} rad", (theta_baseline.abs() - theta_final.abs()));
    println!(" Ghost gradient angles all < 45°: {}", if all_angles_ok { "✓" } else { "✗" });
    println!(" Monotone loss improvement: {}", if monotone_ok { "✓" } else { "~" });
