...
use sundials_core::epirk::{expeuler_step, EpirkConfig};
use sundials_core::mpir::{mpir_solve, MpirConfig, MpirStatus};
...
fn main() {
    ...
    let linear_laplacian = |x: &[Real], out: &mut [Real]| {
        for i in 0..n_grid {
            for j in 0..n_grid {
                ...
                // Laplacian for v
                let lap_v = (x[up + 1] + x[down + 1] + x[left + 1] + x[right + 1] - 4.0 * x[idx + 1]) / (dx * dx);
    ...
    let epirk_cfg = EpirkConfig { krylov_dim: 10, tol: 1e-10 };
    let start = Instant::now();
    // Take 5 macro-steps using EPIRK
    for _ in 0..5 {
        expeuler_step(&linear_laplacian, &nonlinear_react, &mut state, 0.5, &epirk_cfg);
    }
    println!("✅ EPIRK completed 5 steps in {:?}. System smoothly diffused.", start.elapsed());
    ...
    let (du, dv) = gray_scott_reaction(cell_state[0], cell_state[1], f, k);
    let exact_next = vec![cell_state[0] + h * du, cell_state[1] + h * dv];
    
    let err_u = (pred[0] - exact_next[0]).abs();
    let err_v = (pred[1] - exact_next[1]).abs();
    
    println!("   PINN Prediction: u={:.6}, v={:.6}", pred[0], pred[1]);
    println!("   Exact Next:      u={:.6}, v={:.6}", exact_next[0], exact_next[1]);
    println!("   Error:           Δu={:.2e}, Δv={:.2e}", err_u, err_v);
    println!("✅ PINN trained in {:?}. Prediction provides an excellent O(1) starting guess for Newton.", start_pinn.elapsed());
    ...
    let mpir_cfg = MpirConfig { max_iter: 5, tol: 1e-12 };
    let start_mpir = Instant::now();
    let (x, status) = mpir_solve(&a, &b, &mpir_cfg);
    
    match status {
        MpirStatus::Converged { iters, res_norm } => {
            println!("✅ MPIR converged in {} FP64 refinement iterations (norm = {:.2e})", iters, res_norm);
            println!("   Solution: [{:.4}, {:.4}, {:.4}, {:.4}]", x[0], x[1], x[2], x[3]);
