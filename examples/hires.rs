...
fn main() -> Result<(), cvode::CvodeError> {
    ...
    let mut solver = Cvode::builder(Method::Bdf)
        .rtol(1e-3).atol(1e-6).max_steps(50_000_000)
        .build(rhs, 0.0, y0)?;
    println!("HIRES Benchmark (8 species)");
    println!("{:>10} {:>12} {:>12} {:>12} {:>12}","t","y1","y2","y7","y8");
    println!("{}", "-".repeat(62));
    for &tout in &[0.5,1.0,5.0,10.0,50.0,100.0] {
        let (t,y) = solver.solve(tout, Task::Normal)?;
        println!("{t:10.2} {:12.6e} {:12.6e} {:12.6e} {:12.6e}",y[0],y[1],y[6],y[7]);
    }
    let y = solver.y(); let total: f64 = y.iter().sum();
    println!("\nConservation: Σy = {total:.10} (expect 1.0057)");
    println!("Steps: {}, RHS evals: {}", solver.num_steps(), solver.num_rhs_evals());
    Ok(())
}
