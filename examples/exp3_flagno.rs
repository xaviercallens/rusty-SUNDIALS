...
fn anisotropic_ode(kx: f64, ky: f64) -> impl Fn(Real, &[Real], &mut [Real]) -> Result<(), String> {
...
fn run_isotropic_baseline(y0: Vec<f64>) -> (f64, Vec<f64>, u32) {
    ...
    let mut cv = Cvode::builder(Method::Bdf)
        .max_order(1)
        .max_steps(200_000)
        .build(wrapped, 0.0, y0v)
        .unwrap();
    let start = Instant::now();
    let (_, y) = cv
        .solve(0.01, Task::Normal)
        .expect("Isotropic solve failed");
...
fn main() {
    ...
    println!("══════════════════════════════════════════════════════════════");
    println!(
        " Isotropic baseline: {} RHS evals, {:.3}s",
        iters_iso, t_iso
    );
    println!(
        " FLAGNO aligned:     {} RHS evals, {:.3}s",
        iters_gno, t_gno
    );
    ...
    println!(
        " Conservation (both < initial {:.4}): iso={}",
        integ_init,
        if integ_iso <= integ_init + 1e-10 {
            "✓"
        } else {
            "✗"
        }
