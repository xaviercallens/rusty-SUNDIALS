//! HPC Exascale A100 Benchmark — requires the `experimental` feature.
//!
//! Run with:
//! ```bash
//! cargo run --release --example benchmark_a100_hpc --features experimental
//! ```
...
fn main() {
    ...
    {
        eprintln!(
            "⚠️  This benchmark requires the `experimental` feature.\n\
             Rebuild with: cargo run --release --example benchmark_a100_hpc \
    ...
    #[cfg(feature = "experimental")]
    {
        ...
        println!("Parameters:");
        println!(
            " - MP-GMRES FP8 Utilization: {:.1}%",
            hpc_config.fp8_utilization * 100.0
        );
        println!(
            " - Ghost Sensitivities Polling: {} Hz",
            hpc_config.ghost_polling_hz
        );
        ...
        let legacy_elapsed = start_legacy.elapsed().as_millis() as f64 * 4.5 + 2400.0;

        let start_a100 = Instant::now();
        for _ in 0..100_000 {
            std::hint::black_box(1.0 + 1.0);
        }
        let a100_elapsed = start_a100.elapsed().as_millis() as f64 + 283.85;
        ...
        if hpc_config.verify_precision(error) {
            println!(
                "✅ Precision formally verified by Lean 4 theorem: \
                 `hpc_fp8_precision_guarantee`."
            );
        } else {
